...
const ProfilePage = ({ type }) => {
  ...
  useEffect(() => {
    ...
    if (type === "server" && user && access_token) {
      if (activeServer && serverDetails && params.serverId === activeServer) {
        navigate(`/servers/${activeServer}/${serverDetails.channels[0]._id}`);
      }
  ...
  return serverDetails || type === "messages" ? (
